...
#pragma warning disable 168
            var x = instance; // make sure that instance and singleton created?
    ...
    private static PhotonVoiceNetwork instance
    {
        get
        {
            lock (instanceLock)
            {
                ...
                if (_instance == null)
                {
                    _instance = (PhotonVoiceNetwork)FindObjectOfType(typeof(PhotonVoiceNetwork));
                    var objs = FindObjectsOfType(typeof(PhotonVoiceNetwork));
                    if (objs.Length > 1)
                    {
                        Debug.LogError("PUNVoice: Multiple instances of PhotonVoiceNetwork detected " + objs.Length);
                    }
                    else
                    {
                        if (_instance == null)
